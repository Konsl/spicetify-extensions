...
export function initSaveCount() {
	...
	heartRendererEntry.type = function (params) {
		...
		const onLabelClick = Spicetify.React.useCallback(() => {
			const target = `/collection/tracks?uri=${encodeURIComponent(state.nextEntry)}`;

			const isOnTracksPage = Spicetify.Platform.History.location?.pathname === "/collection/tracks";
