...
export class SpotifyModules {
	...
	public static getMetadataService(): unknown | null {
		return this.getValue(
			"metadataService",
			m => m && typeof m === "function" && "SERVICE_ID" in m && m.SERVICE_ID === "spotify.mdata_esperanto.proto.MetadataService"
		);
	}
	...
	public static getCreateTransport(): unknown | null {
		return this.getValue(
			"createTransport",
			m => m && typeof m === "function" && m.toString().includes("executeEsperantoCall") && m.toString().includes("cancelEsperantoCall")
		);
	}
