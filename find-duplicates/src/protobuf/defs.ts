...
function skipValue(reader: Reader, baseType: number) {
	switch (baseType & 7) {
		case 5: // Fixed4
			reader.getView(4);
			break;
		case 1: // Fixed8
			...
			break;
		case 0: // VarInt
			reader.getVarIntView();
			break;
		case 2: // LDelim
			const length = Number(reader.getVarInt());
			reader.getView(length);
			break;
...
export const PBBytes: PBValue<Uint8Array> = [PBBaseType.LDelim, view => new Uint8Array(view.buffer, view.byteOffset, view.byteLength)];
	...
	return [
		...
		view => {
			...
			while (!reader.isExhausted()) {
				...
				if (!field || !verifyType(tag, field.value[0])) {
				...
				if (fieldType[2]) {
					const arr = (message[field.name] as any[] | undefined) ?? [];
					(message as any)[field.name] = arr.concat(value);
				} else {
